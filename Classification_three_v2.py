...
def knn_classifier(_train_data_vector, _train_gt_vector, _valid_data_vector):
    _knn = KNeighborsClassifier(n_neighbors=3)
    _knn.fit(_train_data_vector, _train_gt_vector)
    return _knn.predict(_valid_data_vector)
...
def time_series_aggregate_predictions_classify(_train_data, _train_gt, _valid_data, _valid_gt):
    ...
    for i in range(len(classifier_name)):
        ...
        for time_slice in range(len(_train_data_vector)):
            ...
            _one_predict = classifier_func[i](_slice_train_data, _train_gt_vector, _slice_valid_data)
        ...
        print("accuracy: {}\nprecision: {}\nrecall: {}\nf1 Score: {}".format(
            ...
            precision_score(_final_predict, _valid_gt_vector, average=None),
            recall_score(_final_predict, _valid_gt_vector, average=None),
            f1_score(_final_predict, _valid_gt_vector, average=None)))
        print('memory: %.4f GB' % (psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024))
        print('time: {} s'.format(time.time() - _begin_time))
